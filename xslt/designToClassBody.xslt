...
<xsl:for-each select="d:cachevariable">
  <xsl:value-of select="@dataType"/><xsl:text> </xsl:text> <xsl:value-of select="$className"/>::read<xsl:value-of select="fnc:capFirst(@name)"/> (UaStatus *out_status)
  {
    ...
    UaNodeId nodeId ( UaString(m_objId.identifierString()) + UaString(".<xsl:value-of select="@name"/>"), 2  );
  ...
  void  <xsl:value-of select="$className"/>::write<xsl:value-of select="fnc:capFirst(@name)"/> (<xsl:value-of select="@dataType"/> &amp; data)
  {
  	ServiceSettings   ss;
    ...
    v.copyTo( &amp;nodesToWrite[0].Value.Value );
  	...
  	if (results[0] != OpcUa_Good)
  		throw std::runtime_error(std::string("OPC-UA write failed: one of results not good: ") + UaStatus(results[0]).toString().toUtf8() );
